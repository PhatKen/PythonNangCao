...
def index():
    ...
    products_table = Table('CacLaptop', metadata, autoload_with=engine)
...
def register():
    if request.method == 'POST':
        ...
        engine = get_engine()  # Sử dụng get_engine từ utils.py
        metadata = MetaData()
        users_table = Table('nguoidung', metadata, autoload_with=engine)
...
def get_cart():
    ...
    engine = get_engine()
    metadata = MetaData()
    carts_table = Table('cart', metadata, autoload_with=engine)
    products_table = Table('CacLaptop', metadata, autoload_with=engine)
    ...
    try:
        with engine.connect() as connection:
            query = select(
                carts_table.c.id,
                carts_table.c.laptop_id,
                carts_table.c.total_item,
                products_table.c.laptop,
                products_table.c.gia_tien,
                products_table.c.img
            ).where(
                (carts_table.c.user_id == user_id) &
                (carts_table.c.laptop_id == products_table.c.id)
            )
            result = connection.execute(query)
            cart_items = [
                {
                    "cart_id": row.id,
                    "prod_id": row.id,
                    "prod_qty": row.total_item,
                    "prod_name": row.laptop,
                    "prod_price": row.gia_tien,
    ...
    try:
        with engine.connect() as connection:
            ...
            query = select(carts_table).where(
                (carts_table.c.user_id == user_id) &
                (carts_table.c.laptop_id == prod_id)
            )
            ...
            if existing_cart_item:
                ...
                ).values(prod_qty=carts_table.c.total_item + prod_qty) 
...
def search():
    ...
    query = products_table.select().where(products_table.c.laptop.ilike(f'%{search_query}%'))
...
def show_category(id, category_name):
    ...
    with engine.connect() as connection:
        ...
        query = select(products_table).where(
            products_table.c.danhsach_id == id,
        ).limit(per_page).offset(offset)
        result = connection.execute(query).mappings()
        products = [dict(row) for row in result]

        # Tính tổng số trang
        total_products_query = select(func.count(products_table.c.id)).where(products_table.c.danhsach_id == id)
        total_products_result = connection.execute(total_products_query).scalar()
        total_pages = ceil(total_products_result / per_page)
...
def admin_board():
    ...
    engine = create_engine(f'postgresql+psycopg2://postgres:phat0938581379@localhost:5432')
    try:
        with engine.connect() as connection:
            databases = ["Laptop"]
    except SQLAlchemyError as e:
